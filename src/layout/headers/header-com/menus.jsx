...
const Menus = ({ productsFeatured }) => {
  ...
  // console.log("MenuProduct", productsFeatured);
  ...
  if (productsFeatured?.length > 0) {
    const product_items = productsFeatured;
    // console.log("MenuProduct", product_items);

    content = (
      <div className="row">
        {product_items?.slice(0, 4).map((item) => {
          console.log("MenuProduct", item);
          return (
            <div key={item._id} className="col-md-3">
              {/* <ProductItem prd={item} /> */}
              <p>{item._id}</p>
            </div>
          );
        })}
